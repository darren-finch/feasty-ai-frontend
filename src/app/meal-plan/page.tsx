...
import type MealPlan from "../../lib/models/MealPlan"
    ...
    return (
        <div className="flex flex-col justify-between min-h-screen bg-gray-50">
            ...
            <nav className="bg-white shadow-lg py-4 px-8 flex items-center justify-between border-b border-gray-200">
                ...
                <div className="flex space-x-4">
                    {authUser && (
                        <div>
                            <img
                                src={authUser.image ? authUser.image : undefined}
                                alt="Profile Picture"
                ...
                <div className="w-full max-w-2xl mb-6">
                    {mealPlanData.meals.length === 0 ? (
                        <p className="text-gray-600 text-center">
                            Click the generate button to generate your first meal plan
                        </p>
                    ) : (
                                        ...
                                        <>
                                            <div className="flex flex-col mb-4">
                                                <p className="text-gray-600">
                                                    Total Calories: {macros.calories.toFixed(2)}
                                                </p>
                                                ...
                                                {meal.foods.map((food, foodIndex) => {
                                                    ...
                                                    return (
                                                        <div
                                                            ...
                                                            <p className="text-gray-600">
                                                                {food.name} - {food.descriptor || ""} (
                                                                {food.desiredAmount}
                                                                {food.unit})
                                                            </p>
                                                            <p className="text-gray-600">
                                                                {(food.unitCalories * factor).toFixed(2)}Cal |{" "}
                                                                {(food.unitCarbs * factor).toFixed(2)}C |{" "}
                                                                {(food.unitFats * factor).toFixed(2)}F |{" "}
                                                                {(food.unitProtein * factor).toFixed(2)}P
                                                            </p>
                                                        </div>
                                                    )
                                                })}
            ...
            <footer className="w-full border-t border-gray-200 py-4 text-center bg-white">
                <p className="text-sm text-gray-500">Copyright @2024 Feasty AI - Powered by ChatGPT</p>
            </footer>
        </div>
    )
}
